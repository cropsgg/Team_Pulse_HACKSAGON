...
async function bootstrap() {
  ...
  const httpPort = configService.get('PORT', 4001);
  await app.listen(httpPort);
  console.log(`🔐 Auth service (HTTP/GraphQL) running on port ${httpPort}`);
  ...
  console.log('🔐 Auth service (NATS) connected to message broker');
}
